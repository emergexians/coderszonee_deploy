...
async function ensureUniqueSlug(base: string, excludeId?: any) {
  ...
  while (true) {
    ...
    const exists = await CareerPath.exists(
...
export async function GET() {
  try {
    await dbConnect();
    const list = await CareerPath.find().sort({ createdAt: -1 }).lean({ virtuals: true });
...
export async function POST(req: Request) {
  try {
    ...
    const createdFull = await CareerPath.findById(created._id).lean({ virtuals: true });
