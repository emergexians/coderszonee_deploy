...
async function ensureUniqueSlug(base: string, excludeId?: string): Promise<string> {
  ...
  for (;;) {
    ...
    const exists = await CareerPath.exists(filter);
...
export async function GET() {
  try {
    await dbConnect();
    const list = await CareerPath.find()
      .sort({ createdAt: -1 })
      .lean({ virtuals: true });
    return NextResponse.json(list, { headers: { "cache-control": "no-store" } });
  } catch (err: unknown) {
    console.error("[careerpaths][GET]", err);
...
export async function POST(req: Request) {
  try {
    ...
    const norm: CareerPathInput = { ...(raw as Record<string, unknown>) };
    ...
    const href = `/careerpath/${slug}`;

    // Compose doc with derived fields
    const toCreate = { ...parsed.data, slug, href };

    const created = await CareerPath.create(toCreate);
    const createdFull = await CareerPath.findById(created._id).lean({
      virtuals: true,
    });
    ...
    console.error("[careerpaths][POST]", err);
    const message =
