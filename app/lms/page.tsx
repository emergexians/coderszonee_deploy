      ...
      <header className="sticky top-0 z-40 border-b border-gray-200/70 bg-white/70 backdrop-blur dark:border-gray-800 dark:bg-gray-900/70">
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center gap-3">
          ...
          <div className="ml-3 inline-flex rounded-xl border border-gray-200 dark:border-gray-800 p-1 text-sm">
            <button onClick={() => setRole("student")} className={`rounded-lg px-3 py-1.5 ${role === "student" ? "bg-gray-900 text-white dark:bg-gray-100 dark:text-gray-900" : "hover:bg-gray-50 dark:hover:bg-gray-800/60"}`}>Student</button>
            <button onClick={() => setRole("instructor")} className={`rounded-lg px-3 py-1.5 ${role === "instructor" ? "bg-gray-900 text-white dark:bg-gray-100 dark:text-gray-900" : "hover:bg-gray-50 dark:hover:bg-gray-800/60"}`}>Instructor</button>
          </div>
          <div className="ml-auto flex items-center gap-2">
            <div className="relative hidden md:block">
              <Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-gray-400" />
              <input placeholder="Search courses, tasks, messages" className="w-80 rounded-xl border border-gray-200 bg-white/80 pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/40 dark:bg-gray-900/60 dark:border-gray-800" />
            </div>
            <button className="rounded-xl p-2 hover:bg-gray-100 dark:hover:bg-gray-800" aria-label="Notifications"><Bell className="h-5 w-5" /></button>
            <button className="rounded-xl p-2 hover:bg-gray-100 dark:hover:bg-gray-800" aria-label="Settings"><Settings className="h-5 w-5" /></button>
            <button onClick={() => setTheme((t) => (t === "light" ? "dark" : "light"))} className="rounded-xl p-2 hover:bg-gray-100 dark:hover:bg-gray-800" aria-label="Toggle theme">{theme === "dark" ? <Sun className="h-5 w-5" /> : <Moon className="h-5 w-5" />}</button>
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-6 grid xl:grid-cols-[300px,1fr] gap-6">
      ...
      <aside className="space-y-4">
        <Card>
          <div className="flex items-start justify-between gap-3">
            ...
            <span className="rounded-lg bg-gradient-to-r from-orange-500 to-fuchsia-500 px-2.5 py-1 text-xs text-white">Active</span>
          </div>
      ...
      <section className="space-y-6">
        ...
        <Assignments title="Assignments" items={assignments as any} />
      </section>
    </main>
  );
}
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-6 grid xl:grid-cols-[300px,1fr] gap-6">
      ...
      <aside className="space-y-4">
        ...
        <Card title="Recent Messages">
          <ul className="mt-3 space-y-2 text-sm">
            {[{ id: "M1", from: "Aarav Sharma", course: "CS523", snippet: "Clarify loss function…", at: "2025-08-29T10:12:00Z" }, { id: "M2", from: "Priya Gupta", course: "CS501", snippet: "Absent for lab…", at: "2025-08-28T14:05:00Z" }].map((m) => (
      ...
      <section className="space-y-6">
        ...
        <InstructorTasks title="Grading & Tasks" items={tasks as any} />
      </section>
    </main>
  );
}
...
function Stats({ stats }: { stats: { label: string; value: string | number; icon: React.ReactNode; tone: string }[] }) {
  return (
  ...
  return (
    <motion.div whileHover={{ y: -2 }} className="rounded-xl border border-gray-200 p-4 hover:shadow transition dark:border-gray-800">
      <div className="flex items-center justify-between">
        ...
        <span className="inline-flex items-center gap-1 text-xs text-amber-600"><Star className="h-3.5 w-3.5" /> Featured</span>
      </div>
...
/* ----- Student assignments ----- */
function Assignments({ title, items }: { title: string; items: any[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((a) => {
      ...
      const matchesF = filter === "all" ? true : filter === "due" ? a.status !== "completed" : a.status === "completed";
  ...
  return (
    <Card>
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="relative hidden sm:block">
            <Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-gray-400" />
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search" className="w-56 rounded-xl border border-gray-200 bg-white/80 pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/40 dark:bg-gray-900/60 dark:border-gray-800" />
          </div>
          <div className="inline-flex rounded-xl border border-gray-200 dark:border-gray-800 p-1 text-sm">
            {(["all", "due", "completed"] as const).map((s) => (
              <button key={s} onClick={() => setFilter(s)} className={`rounded-lg px-3 py-1.5 ${filter === s ? "bg-gray-900 text-white dark:bg-gray-100 dark:text-gray-900" : "hover:bg-gray-50 dark:hover:bg-gray-800/60"}`}>{s[0].toUpperCase() + s.slice(1)}</button>
            ))}
          </div>
        </div>
      </div>
          ...
          : filtered.map((a) => (
            <motion.div key={a.id} whileHover={{ y: -1 }} className="rounded-xl border border-gray-200 p-3 dark:border-gray-800">
              <div className="flex items-center justify-between">
                ...
                <Status s={a.status as any} />
              </div>
              <div className="mt-1 text-xs text-gray-500">{a.course} • Due {formatDate(a.due)}</div>
              <div className="mt-2 flex items-center justify-between text-sm">
                <span>Weight {a.weight}%</span>
                <button onClick={() => setOpenId(a.id)} className="rounded-lg border px-2.5 py-1 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800">View</button>
              </div>
            </motion.div>
          ))}
      </div>
      ...
      <div className="mt-2 overflow-x-auto hidden sm:block">
        <table className="min-w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-gray-500"><EmptyState label="No assignments to show." /></td>
              </tr>
            ) : (
              filtered.map((a) => (
                <tr key={a.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/40">
                  ...
                  <Td><Status s={a.status as any} /></Td>
                  <Td>{a.weight}%</Td>
                  <Td className="text-right"><button onClick={() => setOpenId(a.id)} className="rounded-lg border px-2.5 py-1.5 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800">View</button></Td>
                </tr>
              ))
            )}
          </tbody>
      ...
      <AnimatePresence>{openId ? <AssignmentModal key="am" assignment={items.find((x) => x.id === openId)!} onClose={() => setOpenId(null)} /> : null}</AnimatePresence>
    </Card>
  );
}

/* ----- Instructor tasks ----- */
function InstructorTasks({ title, items }: { title: string; items: any[] }) {
  const [filter, setFilter] = useState<"all" | "due" | "graded">("all");
  const [openId, setOpenId] = useState<string | null>(null);

  const filtered = useMemo(() => {
    return items.filter((t) => (filter === "all" ? true : filter === "graded" ? t.status === "graded" : t.status !== "graded"));
  }, [filter, items]);
  ...
  return (
    <Card>
          ...
          filtered.map((t) => (
            <motion.div key={t.id} whileHover={{ y: -1 }} className="rounded-xl border border-gray-200 p-3 dark:border-gray-800">
              <div className="flex items-center justify-between">
                ...
                <Status s={(t.status === "graded" ? "graded" : t.status === "due" ? "due" : "open") as any} />
              </div>
      ...
      <div className="mt-2 overflow-x-auto hidden sm:block">
        <table className="min-w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-gray-500"><EmptyState label="No tasks match your filters." /></td>
              </tr>
            ) : (
              filtered.map((t) => (
                <tr key={t.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/40">
                  ...
                  <Td><Status s={(t.status === "graded" ? "graded" : t.status === "due" ? "due" : "open") as any} /></Td>
                  <Td className="capitalize">{t.type}</Td>
                  <Td className="text-right"><button onClick={() => setOpenId(t.id)} className="rounded-lg border px-2.5 py-1.5 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800">Open</button></Td>
                </tr>
              ))
            )}
          </tbody>
      ...
      <AnimatePresence>{openId ? <TaskModal key="tm" task={items.find((x) => x.id === openId)!} onClose={() => setOpenId(null)} /> : null}</AnimatePresence>
    </Card>
  );
}
...
function Status({ s }: { s: "due" | "open" | "completed" | "graded" }) {
  ...
  const label = s === "graded" ? "Graded" : s === "completed" ? "Completed" : s === "due" ? "Due" : "Open";
  return <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs ${map[s]}`}>{label}</span>;
}
...
function AssignmentModal({ assignment, onClose }: { assignment: { id: string; course: string; title: string; due: string; status: string; weight: number }; onClose: () => void }) {
  return (
      ...
      <div className="absolute inset-0 grid place-items-center p-4">
        ...
        >
          ...
          <div className="mt-4">
            ...
            <p className="mt-1 text-sm leading-relaxed text-gray-700 dark:text-gray-300">Submit a PDF via the LMS. Include your Student ID on the first page. Late submissions up to 48 hours with 10% penalty.</p>
          </div>
      ...
      <div className="absolute inset-0 grid place-items-center p-4">
        ...
        >
          ...
          <div className="mt-4">
            ...
            <p className="mt-1 text-sm leading-relaxed text-gray-700 dark:text-gray-300">Review submissions on the LMS. Apply rubric consistently and publish grades by the due date.</p>
          </div>
          <div className="mt-5 flex items-center justify-end gap-2">
            <button onClick={onClose} className="rounded-lg border border-gray-200 px-3 py-1.5 text-sm hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800">Close</button>
            <button className="rounded-lg bg-gray-900 text-white px-3 py-1.5 text-sm hover:bg-black dark:bg-white dark:text-gray-900">Open in LMS</button>
