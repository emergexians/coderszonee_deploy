 ...
 * - Polished header with gradient bar & theme toggle
 * - Sticky, collapsible sidebar on mobile
 * - Denser cards with subtle motion & better contrast (supports dark mode)
 * - Responsive assignments (cards on mobile, table on desktop)
...
export default function StudentPanel() {
  ...
  const stats = [
    { label: "GPA", value: "8.7", icon: <Trophy className="h-5 w-5" />, tone: "from-emerald-500 to-teal-500" },
    { label: "Credits", value: "82/160", icon: <GraduationCap className="h-5 w-5" />, tone: "from-indigo-500 to-violet-500" },
    ...
    <div className="min-h-[100svh] bg-gradient-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-900 text-gray-900 dark:text-gray-50">
      ...
      <header className="sticky top-0 z-40 border-b border-gray-200/70 bg-white/70 backdrop-blur dark:border-gray-800 dark:bg-gray-900/70">
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center gap-3">
          ...
          <div className="rounded-lg px-3 py-1.5 bg-gray-900 text-white dark:bg-gray-100 dark:text-gray-900">Student Panel</div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="text-sm text-gray-600 mt-2 dark:text-gray-400">{student.program}</div>
                <div className="text-sm text-gray-600 dark:text-gray-400">{student.term}</div>
              </div>
            ...
            <div className="mt-3 h-2 w-full rounded-full bg-gray-100 dark:bg-gray-800">
              <div className="h-2 rounded-full bg-gradient-to-r from-orange-500 to-fuchsia-500" style={{ width: "72%" }} />
            </div>
            <div className="mt-1 text-xs text-gray-500">72% term completed</div>
          </div>
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {courses.map((c) => (
                ...
                >
                  ...
                  <div className="text-sm text-gray-600 mt-1 dark:text-gray-400">{c.faculty}</div>
                  <div className="mt-3 h-2 w-full rounded-full bg-gray-100 dark:bg-gray-800">
                    <div className="h-2 rounded-full bg-gradient-to-r from-sky-500 to-cyan-500" style={{ width: `${c.progress}%` }} />
                  </div>
                  <div className="mt-1 text-xs text-gray-500">{c.progress}% complete</div>
                </motion.div>
              ))}
            </div>
          </div>
...
/* ---------------- components ---------------- */
function NavItem({ icon, text, active, danger }: { icon: React.ReactNode; text: string; active?: boolean; danger?: boolean }) {
  return (
    <button
...
function StatusBadge({ s }: { s: "due" | "open" | "completed" }) {
  const tone =
    s === "completed"
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center gap-2 py-6 text-center">
      ...
      <div className="text-sm text-gray-600 dark:text-gray-400">No items to show.</div>
    </div>
  );
}
        ...
        >
          ...
          <div className="mt-4">
            ...
            <p className="mt-1 text-sm leading-relaxed text-gray-700 dark:text-gray-300">
              Submit a PDF via the LMS. Include your Student ID on the first page. Late
              submissions up to 48 hours with 10% penalty.
            </p>
          </div>
