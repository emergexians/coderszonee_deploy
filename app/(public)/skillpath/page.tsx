...
export default function SkillPathPage() {
  ...
  useEffect(() => {
    const fetchPaths = async () => {
      try {
        const res = await fetch("/api/course/skillpaths", { cache: "no-store" });
        const json = await res.json();
        // API returns { data: SkillPath[], ... }
        const items = Array.isArray(json) ? json : json?.data;
        setSkillPaths(Array.isArray(items) ? items : []);
      } catch (err) {
        console.error("Error fetching skill paths:", err);
        setSkillPaths([]);
      } finally {
        setLoading(false);
      }
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-gray-500 dark:text-gray-400">
            Pick a skill track and follow a structured roadmap to get job-ready.
          </p>
        </div>
