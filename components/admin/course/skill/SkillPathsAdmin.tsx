...
function UrlOrUpload({
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const res = await fetch("/api/upload/skillpaths", { method: "POST", body });
...
/* =========================
   Main Component (Skill Paths)
========================= */
export default function SkillPathsAdmin() {
  const [skill, setSkill] = useState<SkillPath[]>([]);
  ...
  async function confirmDelete() {
    ...
    try {
      const res = await fetch(`/api/course/skillpaths/${id}`, { method: "DELETE" });
      if (!res.ok && res.status !== 204) throw new Error("Delete failed");
      setSkill((p) => p.filter((x) => x._id !== id));
      showToast("Skill Path deleted", "success");
    } catch (e) {
      console.error(e);
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/course/skillpaths", {
        method: "POST",
  ...
  async function startEdit(id: string) {
    ...
    try {
      ...
      console.error(e);
      showToast("Failed to load skill path", "error");
      setOpenEdit(false);
      setEditingId(null);
    } finally {
  ...
  async function handleUpdate(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/course/skillpaths/${editingId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
              ...
              <tbody className="divide-y divide-gray-200">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={6}>
                      <div className="py-12 text-center text-sm text-gray-500">
                        ...
                        No skill paths yet. Click “Add Skill Path”.
                      </div>
                    </td>
                  </tr>
                ) : (
                  filteredSkills
                    ...
                    .map((item) => <RowSkill key={item._id} item={item} />)
                )}
              </tbody>
            </table>
          </div>
...
function Modal({
  ...
  // body scroll lock + Esc handler (parity with CoursesAdmin)
  useEffect(() => {
    if (!open) return;
